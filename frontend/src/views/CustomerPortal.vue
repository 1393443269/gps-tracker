<template>
  <div style="height:100vh;display:flex;flex-direction:column;background:#f0f2f5;">
    ...
    <div style="height:52px;background:#1a2a4a;display:flex;align-items:center;padding:0 20px;gap:16px;flex-shrink:0;">
      ...
      <div style="flex:1;" />
      <el-tag type="success" size="small" effect="dark">{{ devices.length }} 台设备</el-tag>
      <el-button size="small" @click="logout" style="margin-left:8px;">退出</el-button>
    </div>

    <!-- Tab 导航栏 -->
    <div style="background:#fff;border-bottom:1px solid #e4e7ed;flex-shrink:0;display:flex;padding:0 20px;">
      <div v-for="t in tabs" :key="t.key"
        class="portal-tab" :class="{ active: activeTab === t.key }"
    ...
    <div v-show="activeTab === 'realtime'" style="flex:1;display:flex;overflow:hidden;">
      ...
      <div style="width:260px;background:#fff;border-right:1px solid #e4e7ed;display:flex;flex-direction:column;flex-shrink:0;">
        <div style="padding:12px 14px;border-bottom:1px solid #f0f0f0;font-size:13px;font-weight:600;color:#303133;">
          我的设备
          <el-button size="small" text :icon="Refresh" style="float:right;" @click="loadDevices" :loading="devLoading" />
        </div>
        <div style="flex:1;overflow-y:auto;">
          <div v-if="!devices.length && !devLoading"
            style="text-align:center;color:#ccc;padding:40px 0;font-size:13px;">暂无设备</div>
          <div v-for="d in devices" :key="d.phone"
            class="device-item" :class="{ active: selected?.phone === d.phone }"
            ...
            <div v-if="d.last_location_time" style="font-size:11px;color:#bbb;margin-top:4px;margin-left:16px;">
              最后上报: {{ d.last_location_time?.slice(5,16) }}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style="flex:1;position:relative;display:flex;">
        <div id="portal-map" style="flex:1;position:relative;" />

        <!-- 设备详情侧边栏 -->
        <transition name="slide-detail">
          <div v-if="selected" style="width:260px;background:#fff;border-left:1px solid #e4e7ed;overflow-y:auto;flex-shrink:0;">
            ...
            <div style="padding:14px;">
              ...
              <div style="margin-top:14px;display:flex;gap:8px;flex-wrap:wrap;">
                <el-button size="small" type="primary" @click="locateOnMap(selected)">
                  在地图定位
                </el-button>
                <el-button size="small" @click="goTrack(selected.phone)">
                  查看轨迹
                </el-button>
              </div>
            </div>
    ...
    <div v-show="activeTab === 'track'" style="flex:1;display:flex;overflow:hidden;">
      <div style="width:260px;background:#fff;border-right:1px solid #e4e7ed;display:flex;flex-direction:column;flex-shrink:0;padding:14px;gap:10px;overflow-y:auto;">
        <div style="font-size:13px;font-weight:600;color:#303133;">轨迹回放</div>
        <el-form label-width="56px" size="small">
          <el-form-item label="设备">
            <el-select v-model="trkPhone" placeholder="选择设备" style="width:100%;" @change="resetTrack">
        ...
        <el-progress v-if="trkPoints.length" :percentage="trkProgress"
          :status="trkPlaying?'':'success'" />
      </div>
      <div style="flex:1;position:relative;">
        <div id="portal-track-map" style="position:absolute;inset:0;" />
      </div>
    </div>

    <!-- ══ Tab 3：报警记录 ══ -->
    <div v-show="activeTab === 'alarms'" style="flex:1;display:flex;flex-direction:column;overflow:hidden;padding:16px;gap:12px;">
      <div style="display:flex;align-items:center;gap:12px;">
        <span style="font-size:14px;font-weight:600;color:#303133;">报警记录</span>
    ...
    <!-- ══ Tab 4：操作指令 ══ -->
    <div v-show="activeTab === 'cmd'" style="flex:1;display:flex;overflow:hidden;gap:0;">
      <!-- 左：发送指令 -->
      <div style="width:300px;background:#fff;border-right:1px solid #e4e7ed;padding:16px;display:flex;flex-direction:column;gap:14px;flex-shrink:0;">
        <div style="font-size:13px;font-weight:600;color:#303133;">发送指令</div>

        <el-form label-width="56px" size="small">
          <el-form-item label="设备">
            <el-select v-model="cmdPhone" placeholder="选择设备" style="width:100%;">
              <el-option v-for="d in devices" :key="d.phone" :label="d.name||d.phone" :value="d.phone">
        ...
        <div>
          ...
          <div style="display:flex;gap:6px;flex-wrap:wrap;">
            <el-tag v-for="q in quickCmds" :key="q.label" class="quick-cmd"
              @click="cmdText=q.cmd" style="cursor:pointer;">
              {{ q.label }}
            </el-tag>
          </div>
        </div>
        ...
        <el-button type="primary" @click="sendCmd" :loading="cmdSending"
          :disabled="!cmdPhone||!cmdText" style="width:100%;">
          发送
        </el-button>
      ...
      <div style="flex:1;display:flex;flex-direction:column;padding:16px;gap:12px;overflow:hidden;">
        <div style="display:flex;align-items:center;gap:12px;">
          <span style="font-size:14px;font-weight:600;color:#303133;">指令历史</span>
        ...
        <el-table :data="cmdHistory" size="small" style="flex:1;" height="100%" stripe border>
          ...
          <el-table-column label="指令内容" prop="command" min-width="160" />
...
const customerName = ref(JSON.parse(localStorage.getItem('customer_info') || '{}').name || '客户')
...
onMounted(async () => {
  ...
  realtimeMap = new maplibregl.Map({
    container: 'portal-map', style: TDT_MAP_STYLE,
  ...
  trackMap = new maplibregl.Map({
    container: 'portal-track-map', style: TDT_MAP_STYLE,
    center: [104.19, 35.86], zoom: 5,
  })
...
async function loadDevices() {
  ...
  try {
    const res  = await portalApi.devices()
    const list = res.data || []
    devices.value = list
    renderMarkers(list)
    if (list.length && !selected.value) fitAll(list)
...
function fitAll(list) {
  ...
  if (pts.length === 1) {
    realtimeMap.flyTo({ center: [pts[0].last_lng, pts[0].last_lat], zoom: 13, duration: 600 })
    return
  }
...
function goTrack(phone) {
  trkPhone.value = phone
  switchTab('track')
}
...
function resetTrack() {
  pauseTrack()
  trkPoints.value = []; trkIdx.value = -1
...
async function loadAlarms(reset = true) {
  ...
  try {
    const res = await portalApi.alarms({ page: alarmPage.value, size: alarmPageSize.value })
...
async function sendCmd() {
  ...
  try {
    await portalApi.sendCommand({ phone: cmdPhone.value, text: cmdText.value })
...
async function loadCmdHistory(reset = true) {
  ...
  try {
    const res = await portalApi.cmdHistory({ page: cmdHisPage.value, size: cmdHisPageSize.value })
...
.portal-tab:hover  { color: #409EFF; }
.portal-tab.active { color: #409EFF; border-bottom-color: #409EFF; font-weight: 600; }
...
.device-item.active { background: #ecf5ff; }

.quick-cmd:hover { opacity: 0.8; }
